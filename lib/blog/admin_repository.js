...
import {
  get_blog_collection,
  serialize_blog_admin_planning_fields,
  serialize_blog_post,
} from "@/lib/blog/db";
import {
  is_valid_object_id,
...
import { normalize_keyword_theme } from "@/lib/seo/content_planning";
...
export async function admin_get_blog_post_by_id(id) {
  ...
  return {
    ...serialize_blog_post(document),
    ...serialize_blog_admin_planning_fields(document),
  };
}

/**
 * Non-blocking exact-match lookup: does another post already use this exact
 * primary_keyword? Editorial reminder only — never blocks draft/publish.
 * Callers must invoke require_admin() before this (see lib/admin/actions.js).
 */
export async function admin_find_primary_keyword_conflicts(
  primary_keyword,
  exclude_id = null
) {
  ...
  const collection = await get_blog_collection();
  const filter = { primary_keyword: { $exists: true, $ne: null } };
  if (exclude_id && is_valid_object_id(exclude_id)) {
    filter._id = { $ne: new ObjectId(exclude_id) };
  }

  const candidates = await collection
    .find(filter, { projection: { title: 1, slug: 1, status: 1, primary_keyword: 1 } })
    .limit(200)
    .toArray();

  return candidates
    .filter(
      (doc) => normalize_keyword_theme(doc.primary_keyword) === normalized
    )
    .map((doc) => ({
      id: String(doc._id),
      title: doc.title || "",
