...
/**
 * Admin-only editorial planning fields. These must NEVER be merged into the
 * public serializer above, spread from the raw MongoDB document into any
 * public-facing object, or included in metadata/structured data. Merge this
 * only inside an authenticated Admin read path (see
 * lib/blog/admin_repository.js).
 */
export function serialize_blog_admin_planning_fields(document) {
  if (!document) return null;

  return {
    primary_keyword: document.primary_keyword || null,
    search_intent: document.search_intent || null,
    content_cluster: document.content_cluster || null,
    target_service: document.target_service || null,
    target_industry: document.target_industry || null,
  };
}
