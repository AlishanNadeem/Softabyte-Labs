...
import {
  is_valid_content_cluster,
  is_valid_search_intent,
...
/**
 * Sanitize the optional, admin-only SEO planning fields. These are never
 * required and never affect draft/publish validity — see
 * docs/admin_cms_architecture.md for the serialization boundary that keeps
 * them out of public output.
 */
export function sanitize_seo_planning(raw = {}) {
  const primary_keyword = String(raw.primary_keyword || "")
    .trim()
    .slice(0, 160);

  const search_intent_raw = String(raw.search_intent || "").trim();
  const search_intent = is_valid_search_intent(search_intent_raw)
    ? search_intent_raw
    : null;

  const content_cluster_raw = String(raw.content_cluster || "").trim();
  const content_cluster = is_valid_content_cluster(content_cluster_raw)
    ? content_cluster_raw
    : null;

  const target_service_raw = String(raw.target_service || "").trim();
  const target_service = allowed_service_hrefs.has(target_service_raw)
    ? target_service_raw
    : null;

  const target_industry_raw = String(raw.target_industry || "").trim();
  const target_industry = allowed_industry_hrefs.has(target_industry_raw)
    ? target_industry_raw
    : null;
...
export function validate_blog_payload(raw = {}, mode = "draft", options = {}) {
  ...
  const seo_planning = sanitize_seo_planning(raw);
