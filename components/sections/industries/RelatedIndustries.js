...
import { get_industry_by_slug } from "@/config/industries_content";
import { industry_icon_map } from "@/lib/industry_icons";

export function RelatedIndustries({
  eyebrow = "Related industries",
  headline = "Other business contexts that often connect",
  slugs = [],
}) {
  const industries = slugs.map(get_industry_by_slug).filter(Boolean);
    ...
    <Reveal>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-5">
        {industries.map((industry) => {
          const Icon = industry_icon_map[industry.slug];
          return (
            <Link
              key={industry.slug}
              href={industry.path}
